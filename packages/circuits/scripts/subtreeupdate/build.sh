...
# start=`date +%s`
# node "$BUILD_DIR"/"$CIRCUIT_NAME"_js/generate_witness.js "$BUILD_DIR"/"$CIRCUIT_NAME"_js/"$CIRCUIT_NAME".wasm "$SCRIPT_DIR"/input_"$CIRCUIT_NAME".json "$BUILD_DIR"/witness.wtns
# end=`date +%s`
# echo "DONE ($((end-start))s)"
...
# start=`date +%s`
# npx snarkjs zkey beacon "$OUTPUT_DIR"/"$CIRCUIT_NAME"_0.zkey "$OUTPUT_DIR"/"$CIRCUIT_NAME".zkey 0102030405060708090a0b0c0d0e0f101112231415161718221a1b1c1d1e1f 10 -n="Final Beacon phase2"
# end=`date +%s`
# echo "DONE ($((end-start))s)"
...
# start=`date +%s`
# # ~/rapidsnark/build/prover "$OUTPUT_DIR"/"$CIRCUIT_NAME".zkey "$BUILD_DIR"/witness.wtns "$BUILD_DIR"/proof.json "$BUILD_DIR"/public.json
